...
from .file import load_training_trainable_output, save_trainable_robustness_output
from .circuit import trainable_encoding_predict, calculate_accuracy
from .data_types import RobustnessOutput
from .training_trainable import lambdas
...
def calculate_accuracies(x_test, y_test, weights, biases, noise_level, reg, eps):
    x_test = x_test.copy()
    ...
    for i_ensamble in range(n_ensamble):
        ...
        predictions = trainable_encoding_predict(weights, biases, x_disturbed)
...
def run():
    ...
    for i_reg in range(n_lambda):
        ...
        biases_best = training_output.biases_over_epochs[i_reg, best_ensamble_idx, best_epoch_idx]
        lip_best = training_output.lipschitz_bound_over_epochs[i_reg, best_ensamble_idx, best_epoch_idx]
        lip_bounds.append(lip_best)

        for i_noise, noise_level in enumerate(noise_levels):
            job_index = i_reg * (n_noise) + i_noise
            job = dask.delayed(calculate_accuracies)(x_test, y_test, weights_best, biases_best, noise_level, current_lambda, eps)
            jobs.append(job)
    ...
    output = RobustnessOutput(lambdas_to_test,
                              noise_levels,
                              accuracies,
                              lip_bounds,
                              x_test,
    ...
    save_trainable_robustness_output(output)
